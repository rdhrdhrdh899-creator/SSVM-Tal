    ...
    <>
      <nav className={`fixed left-0 right-0 z-50 transition-all duration-500 ease-in-out ${
        isHome
          ? isBannerActive
            ? scrolled
              ? 'top-10 mt-2 mx-4 md:mx-8 rounded-xl md:rounded-full border border-gold-400/20 shadow-2xl glass-nav py-2'
              : 'top-10 bg-transparent py-4'
            : scrolled
              ? 'top-0 mt-4 mx-4 md:mx-8 md:mt-6 rounded-xl md:rounded-full border border-gold-400/20 shadow-2xl glass-nav py-2'
              : 'top-0 bg-transparent py-4'
          : isBannerActive
            ? scrolled
              ? 'top-10 mt-2 mx-4 md:mx-8 rounded-xl md:rounded-full border border-gold-400/10 shadow-xl bg-white/95 backdrop-blur-md py-2'
              : 'top-10 bg-white border-b border-gray-200/50 shadow-sm py-4'
            : scrolled
              ? 'top-0 mt-4 mx-4 md:mx-8 md:mt-6 rounded-xl md:rounded-full border border-gold-400/10 shadow-xl bg-white/95 backdrop-blur-md py-2'
              : 'top-0 bg-white border-b border-gray-200/50 shadow-sm py-4'
      }`}>
        ...
        <Link to="/" className="flex items-center space-x-3">
          ...
          <div className="hidden sm:block">
            <span className={`heading-serif block leading-none font-bold text-lg uppercase ${hasDarkBg ? 'text-white' : 'text-navy-950'}`}>{settings?.schoolName || SCHOOL_NAME}</span>
        ...
        <div className="hidden lg:flex items-center space-x-1 lg:space-x-0.5 xl:space-x-1">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              className={`relative px-2 xl:px-4 py-2 text-[13px] xl:text-sm font-medium transition-colors hover:text-gold-500 group ${location.pathname === link.path ? 'text-gold-500' : hasDarkBg ? 'text-white/90' : 'text-navy-900'}`}
            >
              {link.name}
        ...
        <div className="hidden md:flex items-center space-x-2 xl:space-x-3">
          {isAuthenticated ? (
            ...
            <>
              <Link to="/login">
                <Button variant="outline" size="sm" className={`rounded-full px-4 xl:px-5 transition-colors ${hasDarkBg ? 'border-white/30 text-white hover:bg-white/10' : 'border-navy-800 text-navy-800'}`}>Portal Login</Button>
              </Link>
              <Link to="/admissions">
        ...
        <button
          ...
          className={`lg:hidden p-2 rounded-lg transition-colors ${hasDarkBg ? 'text-white hover:bg-white/10' : 'text-navy-900 hover:bg-gold-50'}`}
        >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-white z-40 lg:hidden p-6 overflow-y-auto"
            style={{
              top: isBannerActive 
                ? scrolled ? '125px' : '120px' 
                : scrolled ? '85px' : '80px'
            }}
          >
